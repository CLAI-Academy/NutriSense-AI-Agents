...
def nutrition_target_agent():
    """
    Agente para calcular objetivos nutricionales basado en datos del usuario
    """
    model = get_chat_model(model_type="gpt", temperature=0.3).with_structured_output(NutritionTargetSchema)
    prompt = ChatPromptTemplate([
        ("system", USER_PROFILE_TARGETS_PROMPT),
        ("human", 
        """
        Edad: {age}
        Género: {gender}
...
def get_user_profile_agent_chain():
    """
...
def get_profile_summary_agent():
    """
    Agente para generar resumen conversacional del perfil nutricional
    """
    model = get_chat_model(model_type="gpt", temperature=0.3).with_structured_output(ProfileSummarySchema)
    ...
    prompt = ChatPromptTemplate([
        ...
        ("human", 
         ...
         Crea un resumen conversacional que explique este perfil de manera personal y motivadora.
         """
        )
    ])
